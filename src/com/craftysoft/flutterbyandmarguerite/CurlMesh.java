...
public class CurlMesh {
	...
	private static final double BACKFACE_ALPHA = 1f;//.2f;craftyhack
	...
	private Bitmap mBitmap, mBitmapBackside;
	...
	public boolean swapFrontToBack = false;
	...
	public synchronized void draw(GL10 gl) {
		...
		if (DRAW_TEXTURE && mTextureIds == null) {
			...
			mTextureIds = new int[2];//craftyhack
			gl.glGenTextures(2, mTextureIds, 0);//craftyhack
		...
		//craftyhack
		//bind backside texture
		if (DRAW_TEXTURE && mBitmapBackside != null) {
			gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureIds[1]);
			GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, mBitmapBackside, 0);
			mBitmapBackside = null;
		}


		if (DRAW_TEXTURE) {
			
			if(swapFrontToBack)//craftyhack
			{
				int temp = mTextureIds[0];
				mTextureIds[0] = mTextureIds[1];
				mTextureIds[1] = temp;
				swapFrontToBack = false;
			}
		...
		if (DRAW_TEXTURE) {
			...
			gl.glBindTexture(GL10.GL_TEXTURE_2D, mTextureIds[1]);//craftyhack
	...
	public synchronized void setBacksideBitmap(Bitmap bitmap) {
		if (DRAW_TEXTURE) {
			// Bitmap original size.
			int w = bitmap.getWidth();
			int h = bitmap.getHeight();
			...
			// TODO: Is there another way to create a bigger Bitmap and copy
			// original Bitmap to it more efficiently?
			mBitmapBackside = Bitmap.createBitmap(newW, newH, bitmap.getConfig());
			Canvas c = new Canvas(mBitmapBackside);
			c.drawBitmap(bitmap, 0, 0, null);
